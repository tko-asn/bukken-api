...
const findOrCreateCategory = (categoryObj) => {
  ...
  return db.category
    .findOrCreate({
      where: {
        firstCategory: categoryObj.firstCategory,
        secondCategory: categoryObj.secondCategory,
      },
      defaults: {
        firstCategory: categoryObj.firstCategory,
        secondCategory: categoryObj.secondCategory,
...
const categoryController = {
  ...
  gerOrCreateCategorys(req, res, next) {
    Promise.all(
      // 非同期処理の配列をPromise.all()で実行
      req.body.map(
        async (categoryObj) => await findOrCreateCategory(categoryObj)
      )
    )
      .then((values) => {
